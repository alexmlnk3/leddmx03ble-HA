...
class LEDBLELEDLampFlowHandler(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_validate(self, user_input: "dict[str, Any] | None" = None):
        ...
        # УСПЕХ — ОБЯЗАТЕЛЬНО вернуть FlowResult
        return self.async_show_form(
            step_id="validate",
    ...
    async def toggle_light(self):
        try:
            ...
            # BLE-устройство сейчас не видно Bluetooth-интеграции HA -> это нормально
            LOGGER.warning("BLE device not ready/visible during setup: %s", err)
            return "not_found"
