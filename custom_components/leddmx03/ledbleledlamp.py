...
MUSIC_MODE_CMD = {
    1: bytearray.fromhex("7b ff 0b 01 00 ff ff ff bf"),
    2: bytearray.fromhex("7b ff 0b 02 00 ff ff ff bf"),
    3: bytearray.fromhex("7b ff 0b 03 00 ff ff ff bf"),
    4: bytearray.fromhex("7b ff 0b 04 00 ff ff ff bf"),
}

EFFECT_LIST = sorted(EFFECT_MAP)
EFFECT_ID_NAME = {v: k for k, v in EFFECT_MAP.items()}

NAME_ARRAY = ["LEDDMX-03"]
WRITE_CHARACTERISTIC_UUIDS = ["0000ffe1-0000-1000-8000-00805f9b34fb"]
TURN_ON_CMD  = [
  bytearray.fromhex("7b ff 04 01 ff ff ff ff bf")  # LEDDMX-03 (пока так же)
]
TURN_OFF_CMD = [
  bytearray.fromhex("7b ff 04 00 ff ff ff ff bf")
]
DEFAULT_ATTEMPTS = 3
...
class LEDBLELEDLamp:
    ...
    async def set_brightness_local(self, value: int):
        ...
        # В протоколе 03 яркость выглядит как:
        # 7b ff 01 HI LO 00 ff ff bf
        # Подберём шкалу: 0..255 -> 0..0xFFFF (2047)
        level = int(value * 0xFFFF / 255)
        hi = (level >> 8) & 0xFF
        lo = level & 0xFF

        brightness_packet = bytearray([0x7B, 0xFF, 0x01, hi, lo, 0x00, 0xFF, 0xFF, 0xBF])
    ...
    async def set_rgb_color(self, rgb: Tuple[int, int, int], brightness: int | None = None):
        ...
        # LEDDMX-03: CMD 0x07, порядок каналов: G R B, параметр X = 0x00
        pkt = bytearray([0x7B, 0xFF, 0x07, g, r, b, 0x00, 0xFF, 0xBF])
    ...
    async def set_effect(self, effect: str):
        if effect not in EFFECT_LIST:
            ...
            return
        self._effect = effect
        effect_packet = bytearray.fromhex("7b ff 03")
        ...
        effect_packet.extend([0x03, 0xff, 0xff, 0xff, 0xbf])
        await self._write(effect_packet)

    @retry_bluetooth_connection_error
    async def set_music_mode(self, mode: int) -> None:
        mode = int(mode)
        if mode not in MUSIC_MODE_CMD:
            LOGGER.error("Music mode %s not supported (expected 1..4)", mode)
            return

        await self._write(MUSIC_MODE_CMD[mode])
